...
public class DiamondEconomyConfig implements ConfigData {
    ...
    public static Item getCurrency(int num) {
        return Registries.ITEM.get(Identifier.tryParse(DiamondEconomyConfig.getInstance().currencies[num]));
    }

    public static String getCurrencyName(int num) {
        return Registries.ITEM.get(Identifier.tryParse(DiamondEconomyConfig.getInstance().currencies[num])).getName().getString();
    }
